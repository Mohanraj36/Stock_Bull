...
from .models import *
from .helpers import send_forget_password_mail, send_contactus_mail, generateOTP, send_OTP
import uuid
...
def search(request,ticker_value =None):
    try:
        if request.user.is_authenticated:      
            if ticker_value is None:
                return render(request, 'search.html')
            else:
                return render(request, 'search.html', {'ticker_value':ticker_value})
        else:
...
# Machine Learning as well as Plotting the Future market
def predict(request, ticker_value, number_of_days):
    ...
    try:
        ...
        ticker_val = ticker_value
        print("Downloaded ticker = {} successfully".format(ticker_value))
    except:
        return render(request, 'error/API_Down.html', {})
    ...
    # Fetching values from Yahoo Finance API
    ...
    # Prediction Score
    global confidence
    confidence = round(clf.score(X_test, y_test) * 100, 2)
    ...
    # Show Selected Ticker Information
    ...
    user_predict_count = Predictioncount.objects.get(user = user)
    user_predict_count.predictions_count += 1
    user_predict_count.save()
...
def stsave(request):
    try:
        user = User.objects.get(username = request.user.username)
        x = Usersaved.objects.create(user = user)
        x.stock_ticker_value = ticker_val
        x.stock_number_of_days = num_of_days
        x.stock_confidence = confidence
        x.stock_data = pred_dict
        x.save()
        user_predict = Userprediction.objects.create(user = user)
        user_predict.graph_id = x.id
        user_predict.save()
...
def dashboard(request):
    if request.user.is_authenticated:
        try:
            ...
            data = Usersaved.objects.filter(user=request.user.id)
            total_predictions = Predictioncount.objects.aggregate(Sum('predictions_count'))['predictions_count__sum']
            user_total_count = Predictioncount.objects.get(user = request.user.id)
            user_total_count = user_total_count.predictions_count 
            profit_count = Userprediction.objects.filter(user=request.user, profit_loss='profit').aggregate(profit_count=Count('id'))['profit_count']
            loss_count = Userprediction.objects.filter(user=request.user, profit_loss='loss').aggregate(loss_count=Count('id'))['loss_count']

            # Initialize lists to hold data for each entry
            confidence = []
            ticker_value = []
            number_of_days = []
            plot_divs = []
            ...
            # Loop over each entry and extract the required data
            for entry in data:
                d = entry.stock_data
                pred_df = pd.DataFrame(eval(d))
                pred_fig = go.Figure([go.Scatter(x=pred_df['Date'], y=pred_df['Prediction'])])
                pred_fig.update_xaxes(rangeslider_visible=True)
                pred_fig.update_layout(paper_bgcolor="#14151b", plot_bgcolor="#14151b", font_color="white")
                plot_div_pred = plot(pred_fig, auto_open=False, output_type='div')
                confidence.append(entry.stock_confidence)
                ticker_value.append(entry.stock_ticker_value)
                number_of_days.append(entry.stock_number_of_days)
                plot_divs.append(plot_div_pred)
                id.append(entry.id)
            # Pass the data to the template context

            plot_data = zip(id, confidence, number_of_days, ticker_value, plot_divs)

            # Pass the lists of data to the template context
            return render(request, 'dashboard.html', context={'plot_data': plot_data,'count':user_total_count,'over': total_predictions,'loss':loss_count,'profit':profit_count})
        except Exception as e:
            print(e)
            return redirect('/')
...
def delete_graph(request, id):
    try:
        graph = Usersaved.objects.get(id = id)
        graph.delete()
...
def profit(request, id):
    get_graph = Userprediction.objects.get(graph_id = id)
    try:
        if get_graph.button_clicked:
            # Button has already been clicked, do not update
            messages.info(request,'alredy Updated')
        else:
            get_graph.profit_loss = 'profit'
            get_graph.button_clicked = True
            get_graph.save()
        return redirect('dashboard')
    except Exception as e:
        print(e)
...
def tr_info(request):
    try:
        if request.user.is_authenticated:
            return render(request,'trading/info.html', {})
        else:
            return render(request,'auth/unauthorized.html')
    except Exception as e:
        print(e)
...
def tr_overview(request):
    try:
        if request.user.is_authenticated:
            return render(request,'trading/overview.html', {})
        else:
            return render(request,'auth/unauthorized.html')
    except Exception as e:
        print(e)
...
def tr_live(request):
    try:
        if request.user.is_authenticated:
            ...
            return render(request,'trading/live.html', {})
        else:
            return render(request, 'auth/unauthorized.html')
    except Exception as e:
        print(e)
...
def login_page(request):
    try:
        ...
        if request.user.is_authenticated and request.method =='GET':
            messages.info(request,'Your are Alredy Logged In')
            return redirect('/')
        else:
            try:
                if request.method == 'POST':
                    logging_uname = request.POST.get('uname')
                    logging_password = request.POST.get('pass')
                    user = authenticate(
                        request, username=logging_uname, password=logging_password)
                    if user is not None:
                        login(request, user)
                        return redirect('/')
                    else:
...
def reg_page(request):
    try:
        if request.user.is_authenticated:
            messages.info(request,'Your are Alredy Registerd User')
            return redirect('/')
        else:
            try:
                if request.method == 'POST':
                        ...
                        if not User.objects.filter(email = email).exists():
                            if not User.objects.filter(username = uname).exists():
                                if pass1 == pass2:
                                    ...
                                    user_create = Predictioncount.objects.create(user = user)
                                    profile_obj.save()
                                    user_create.save()
                                    ...
                                    return render(request, 'auth/register.html', context={'email':email, 'uname': uname})
                            else:
                                messages.error(request, 'Username Reserved')
...
def change_password(request, token):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            user_obj.save()
            return render(request, 'auth/after-password.html')
    ...
    try:
        if request.method == 'POST':
            ...
            if not User.objects.filter(username = username).first():
                ...
                messages.error(request,'An email is Send.')
                return redirect('/forget-password/')
    except Exception as e:
        print(e)
...
def contactus(request):
    try:
        if request.method == 'POST':
            ...
            send_contactus_mail(uname,email,subject,message)
            messages.info(request,f'Thanks For Contacting Us {uname}')
            return redirect('/')
    except Exception as e:
        print(e)
...
def profile(request):
    try:
        if request.user.is_authenticated:
            ...
            profit_count = Userprediction.objects.filter(user=request.user, profit_loss='profit').aggregate(profit_count=Count('id'))['profit_count']
            loss_count = Userprediction.objects.filter(user=request.user, profit_loss='loss').aggregate(loss_count=Count('id'))['loss_count']
            return render(request, 'profile.html',{'user':user, 'email':email, 'doj': doj,'last_login':last_login,'active':active, 'profit':profit_count,'loss': loss_count})
        else:
            messages.error("Unauthoried Access")
            return render(request,'auth/unauthorized.html')
    except Exception as e:
        print(e)
...
def get_form(request, form_id):
    ...
    try:
        if form_id == 'info':
            ...
            profit_count = Userprediction.objects.filter(user=request.user, profit_loss='profit').aggregate(profit_count=Count('id'))['profit_count']
            loss_count = Userprediction.objects.filter(user=request.user, profit_loss='loss').aggregate(loss_count=Count('id'))['loss_count']
            form_html = render_to_string(f"dynamic/{form_id}.html",{'user':user, 'email':email, 'doj': doj,'last_login':last_login,'active':active, 'profit':profit_count,'loss': loss_count})
            ...
            form_html = f"""
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <a href="../" class="">
                                    <h3 class="text-white"><i class="fa fa-user-lock me-2"></i>Reset Username</h3>
                                </a>
                            </div>
                            <form method="post" action="/reset_username/">
                                <input type="hidden" name="csrfmiddlewaretoken" value="{csrf_token}">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control text-dark" id="curr-uname" placeholder="Current Username" name="curr-uname"
                                        autocomplete="off" required>
                                    <label for="curr-uname">Current Username</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control text-dark" id="new-uname1" placeholder="New Username" name="new-uname1"
                                        autocomplete="off" required>
                                    <label for="new-uname1">New Username</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control text-dark" id="new-uname2" placeholder="Confirm New Username" name="new-uname2"
                                        autocomplete="off" required>
                                    <label for="new-uname2">Confirm New Username</label>
                                </div>
                                <button type="submit" class="btn btn-success py-3 w-100 mb-4">Submit</button>
                            </form>
                        """
            return HttpResponse(form_html)
            ...
            form_html = f"""
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <a href="../" class="">
                                    <h3 class="text-white"><i class="fa fa-user-lock me-2"></i>Reset Password</h3>
                                </a>
                            </div>
                            <form method="post" action="/reset_pass/">
                                <input type="hidden" name="csrfmiddlewaretoken" value="{csrf_token}">
                                <div class="form-floating mb-3">
                                    <input type="password" class="form-control text-dark" id="curr_pass" placeholder="Current Password" name="curr_pass" autocomplete="off" required>
                                    <label for="curr_pass">Current Password</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="password" class="form-control text-dark" id="pass1" placeholder="New Password" name="pass1" autocomplete="off" required>
                                    <label for="pass1">New Password</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="password" class="form-control text-dark" id="pass2" placeholder="Confirm New Password" name="pass2" autocomplete="off" required>
            ...
            form_html = f"""
                            <div class="d-flex align-items-center justify-content-between mb-3">
                                <a href="../" class="">
                                    <h3 class="text-white"><i class="fa fa-user-lock me-2"></i>Reset Email</h3>
                                </a>
                            </div>
                            <form method="post" action="/reset_email/">
                                <input type="hidden" name="csrfmiddlewaretoken" value="{csrf_token}">
                                <input type="hidden" name="user" value="{user.username}">
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control text-dark" id="curr-mail" placeholder="Current Maild Id" name="curr-mail"
                                        autocomplete="off" required>
                                    <label for="curr-mail">Current Mail Id</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control text-dark" id="curr-mail-otp" placeholder="Current Maild Id" name="curr-mail-otp"
                                        autocomplete="off" >
                                    <label for="curr-mail-otp">Current Mail Id OTP</label>
                                </div>
                                <div class="form-floating mb-3">
                                    <input type="text" class="form-control text-dark" id="new-mail" placeholder="New Mail Address" name="new-mail"
                                        autocomplete="off" required>
                                    <label for="new-mail">New Mail Id</label>
                                </div>
                                <button type="submit" class="btn btn-success py-3 w-100 mb-4">Submit</button>
                            </form>
                        """
...
def reset_pass(request):
    try:
        if request.method == 'POST':
            ...
            new_password1 = request.POST.get('pass1')
            new_password2 = request.POST.get('pass2')
...
def reset_username(request):
    try:
        if request.method == 'POST':
            user = User.objects.get(username = request.user)
            curr_username = request.POST.get('curr-uname')
            new_username1 = request.POST.get('new-uname1')
            new_username2 = request.POST.get('new-uname2')
            if curr_username == user.username:
                if new_username1 == new_username2:
                    if user.username != new_username1:
                        user.username = new_username1
                        user.save()
                        messages.error(request,"Username Changed Success.")
                        return redirect('profile')
                    else:
                        messages.error(request,"Existing username is same as New username.")
                        return redirect('profile')
                else:
                    messages.error(request, "New username and confirm username do not match.")
                    return redirect('profile')
            else:
                messages.error(request, "Current Username is Wrong.")
                return redirect('profile')
        else:
            messages.error(request, 'Bad Request')
            return redirect('profile')
...
def reset_email(request):
    try:
        if request.method == 'POST':
            curr_mail = request.POST.get('curr-mail')
            curr_mail_otp = request.POST.get('curr-mail-otp')
            new_mail = request.POST.get('new-mail')                        
            user = User.objects.get(username=request.user)
            profile_user = Profile.objects.get(user=user)
            profile_user_otp = profile_user.reset_email_otp

            if curr_mail != new_mail:
                if curr_mail == user.email:
                    if profile_user_otp == None or "":
                        otp = generateOTP()
                        if send_OTP(otp, user.email):
                            profile_user.reset_email_otp = otp
                            profile_user.reset_email_otp_created_at = timezone.now() + timezone.timedelta(minutes=5)
                            print(profile_user)
                            profile_user.save()
                            messages.success(request, "OTP sent to the registered email address \n OTP Expires In 5 Minutes.")
                        else:
                            messages.error(request, "Failed to send the email.")

                    else:
                        if profile_user.reset_email_otp_created_at <= timezone.now():
                            print(timezone.now())
                            print(profile_user.reset_email_otp)
                            if profile_user_otp == curr_mail_otp:
                                user.email = new_mail
                                user.save()
                                profile_user.reset_email_otp = None
                                profile_user.save()
                            ...
                            profile_user.reset_email_otp = None
                            profile_user.reset_email_otp_created_at = None
                            profile_user.save()
                            messages.error(request, "OTP Expired! Try Again")
                else:
                    messages.error(request, "Enter the current email correctly.")
            else:
                messages.error(request, "Existing email is the same as new email.")
        else:
            messages.error(request, "Bad request.")
